...
warden produces **two independent verdicts per job**, because "is this job wasting a GPU" is really two different questions.

**Activity — what the GPU is doing.** A lifecycle, not a flag:

| State | Meaning |
|---|---|
| `analyzing` | Inside the warmup window, or not enough telemetry to judge. **The safe default** — warden says "not enough data" rather than guessing. |
...
| `possibly under-provisioned` | **Hinted only.** A maxed-out card can mean healthy-and-perfect or starving-for-more, so warden never asserts this. |
...
Where Slurm does not publish `gres_detail`, warden falls back to node-wide scoping and **marks the row with `*`** plus a footnote. It never quietly presents node-wide numbers as if they were the job's own.

---

## The signals warden reads, and why

**DCGM** is NVIDIA's Data Center GPU Manager — the telemetry stack that publishes per-GPU readings. warden reads them from Prometheus via *dcgm-exporter*; it never touches a GPU directly.

| Signal | DCGM field | Unit | Why warden reads it |
|---|---|---|---|
...
| Framebuffer free | `DCGM_FI_DEV_FB_FREE` | **absolute MiB** | Added to `FB_USED` to derive the card's total memory. This is how warden learns it is looking at a 15 GiB card **without being told what hardware it runs on** — which is why the over-provisioned verdict is phrased as an observation, not a hardware recommendation. |
...
**A broken sensor is not an idle fleet.** If `GR_ENGINE_ACTIVE` reads flat on every GPU in the cluster while utilization is high on any of them, warden treats the instrument as faulted, drops the signal for that cycle, and says so. A genuine-looking zero is more dangerous than a missing metric, because it reads as corroboration.
...
These are **global to the warden instance**. There are deliberately no per-job knobs — nobody gets to tune away an inconvenient verdict on their own job.
...
#### Basic Usage
```
lmsilva@PANDAMONIUM:~/gpu-warden$ go run ./cmd/warden
...
lmsilva@PANDAMONIUM:~/gpu-warden$
```

#### Show the evidence
```
lmsilva@PANDAMONIUM:~/gpu-warden$ go run ./cmd/warden --wide
...
```
lmsilva@PANDAMONIUM:~/gpu-warden$ go run ./cmd/warden --watch 30s --dollar-rate 0.53 -wide
...
lmsilva@PANDAMONIUM:~/gpu-warden$ curl -s http://localhost:9410/metrics | grep -E 'warden_job_(activity|sizing)'
# HELP warden_job_activity Job activity verdict: 1 on the state currently held (analyzing, healthy, idle, zombie).
# TYPE warden_job_activity gauge
warden_job_activity{job_id="129",user="uid:50000",partition="all",state="analyzing"} 0
warden_job_activity{job_id="129",user="uid:50000",partition="all",state="healthy"} 1
warden_job_activity{job_id="129",user="uid:50000",partition="all",state="idle"} 0
warden_job_activity{job_id="129",user="uid:50000",partition="all",state="zombie"} 0
warden_job_activity{job_id="130",user="uid:50000",partition="all",state="analyzing"} 1
warden_job_activity{job_id="130",user="uid:50000",partition="all",state="healthy"} 0
warden_job_activity{job_id="130",user="uid:50000",partition="all",state="idle"} 0
warden_job_activity{job_id="130",user="uid:50000",partition="all",state="zombie"} 0
# HELP warden_job_sizing Job sizing verdict: 1 on the state currently held (unknown, right_sized, over_provisioned, under_provisioned).
# TYPE warden_job_sizing gauge
warden_job_sizing{job_id="129",user="uid:50000",partition="all",state="unknown"} 0
warden_job_sizing{job_id="129",user="uid:50000",partition="all",state="right_sized"} 1
warden_job_sizing{job_id="129",user="uid:50000",partition="all",state="over_provisioned"} 0
warden_job_sizing{job_id="129",user="uid:50000",partition="all",state="under_provisioned"} 0
warden_job_sizing{job_id="130",user="uid:50000",partition="all",state="unknown"} 1
warden_job_sizing{job_id="130",user="uid:50000",partition="all",state="right_sized"} 0
warden_job_sizing{job_id="130",user="uid:50000",partition="all",state="over_provisioned"} 0
warden_job_sizing{job_id="130",user="uid:50000",partition="all",state="under_provisioned"} 0
lmsilva@PANDAMONIUM:~/gpu-warden$
```

#### Act on the findings by stamping the POD!
```
lmsilva@PANDAMONIUM:~/gpu-warden$ go run ./cmd/warden --watch 30s --dollar-rate 0.53 --grace 1m --zombie-after 1m --act
...
^C
stopping
lmsilva@PANDAMONIUM:~/gpu-warden$ ZNODE=$(kubectl -n slurm exec slurm-controller-0 -c slurmctld -- squeue -h -n zombie -o %N)
lmsilva@PANDAMONIUM:~/gpu-warden$ ZNODE=$(kubectl -n slurm exec slurm-controller-0 -c slurmctld -- squeue -h -n zombie -o %N)
lmsilva@PANDAMONIUM:~/gpu-warden$ ZPOD=$(kubectl -n slurm get pods -l nodeset.slinky.slurm.net/pod-hostname=$ZNODE -o jsonpath='{.items[0].metadata.name}')
lmsilva@PANDAMONIUM:~/gpu-warden$ kubectl -n slurm describe pod "$ZPOD" | tail -n 15
...
Events:
  ...
  ----     ------             ----   ----        -------
  Warning  GPUAllocationIdle  2m23s  gpu-warden  slurm job 130 (user uid:50000) holds GPUs with no activity: confidence high: idle 4m0s (peak GPU 0% over 30m0s)
lmsilva@PANDAMONIUM:~/gpu-warden$
```
...
| Series | Type | Notes |
|---|---|---|
| `warden_job_gpu_utilization_percent` | gauge | Lifetime average utilization per job. |
| `warden_job_gpu_hours_wasted` | gauge | Allocated-but-unused GPU-hours. |
| `warden_job_activity` | gauge | One series per state, `1` on the state held. |
| `warden_job_sizing` | gauge | One series per state, `1` on the state held. |
| `warden_job_verdict_confidence` | gauge | `0` none, `1` low, `2` medium, `3` high. |
| `warden_job_gpu_memory_peak_bytes` | gauge | Peak framebuffer used on a single GPU. |
| `warden_job_gpu_memory_capacity_bytes` | gauge | That GPU's total framebuffer. |
| `warden_job_zombie` | gauge | Kept for compatibility; derived from `warden_job_activity`. |
...
6. **Augment Slurm; never do its scheduling.** warden states one thing Slurm structurally cannot see: that an allocation is doing no real work.
